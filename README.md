# RPG Statistics Screen for Minecraft Modpack
This Mod is for my modpack, trying to implement Roles and Bonuses etc.
Is supposed to enhance Minecraft in a way where you can experience different Playthroughs instead of the same old Minecraft.

# Leveling System
## Level
Your Character Level will define a few things, first and foremost, what kind of Stats you have, per 2 levels you will get one Stat point, which you can spend however you'd like. (Except larry, he gets one each level.)
You can Increase your level by beating Challenges and fullfilling quests.

It will also affect what kind of Tools, Weapons and Armor you can use. For Example, you won't be able to wear Diamond Armor before level 30.

## Level Progression & Quests
### Level Progression
I'd like to keep progression quite slow, so while Quests and Challenges will be "easy" at the start, they won't give much XP.
Being restricted in both, Armor and Stats will also increase the amount of time they take to Complete.
### Quests and Challenges
The Quests and Challenges in this Mod are based on your level, they usually change in difficulty every 10 Levels but this will decrease in higher Level-Ranges.

#### Quest Examples Level 0-10
- Farm 100 wheat
- Kill 20 Zombies
- Farm 100 Wood-Logs.
- Survive 10 In-game days Without Dying

### Quest Examples Level 95-100
- Kill the enderdragon while having no Armor, or Food.
- Kill 4 Wardens with a Sword in 5 Minutes.
- Fight certain Endboss.
- Figure out the Meaning of life.

# Descriptions
## Roles
### Knight
Increased Damage, Resistance and Access to lots of new Weapons. Slight Health Increase and Slight Dexterity Increase
#### Start Stats - 47 Points
| Name        | Min | Max |
|-------------|-----|-----|
| Health      | 12  | 18  |
| Strength    | 12  | 20  |
| Resistance  | 10  | 15  |
| Dexterity   | 10  | 13  |
| Magic       | 0   | 5   |
| Ranged      | 6   | 10  |
| Hunger      | 10  | 15  |
| Adaptabilty | 11  | 18  |

### Mage
Slightly decreased Health, Strength, Dexterity and Resistance, Magic Level 10 (Can use Magic Instantly)
#### Start Stats - 44 Points
| Name        | Min | Max |
|-------------|-----|-----|
| Health      | 7   | 14  |
| Strength    | 4   | 8   |
| Resistance  | 7   | 12  |
| Dexterity   | 10  | 15  |
| Magic       | 10  | 20  |
| Ranged      | 4   | 8   |
| Hunger      | 10  | 15  |
| Adaptabilty | 8   | 12  |

### Assasin
Increased Strength and Dexterity, Slight Ranged Buff. All Other Stats (Health, Resistance and Magic) decreased.
#### Start Stats - 41 Points
| Name        | Min | Max |
|-------------|-----|-----|
| Health      | 7   | 14  |
| Strength    | 6   | 13  |
| Resistance  | 10  | 12  |
| Dexterity   | 15  | 25  |
| Magic       | 2   | 8   |
| Ranged      | 8   | 12  |
| Hunger      | 10  | 15  |
| Adaptabilty | 12  | 16  |

### Archer
Ranged and Dexterity buff, rest of the Stats decreased slightly no Magic
#### Start Stats - 48 Points
| Name        | Min | Max |
|-------------|-----|-----|
| Health      | 7   | 14  |
| Strength    | 5   | 10  |
| Resistance  | 6   | 10  |
| Dexterity   | 10  | 20  |
| Magic       | 2   | 8   |
| Ranged      | 12  | 18  |
| Hunger      | 10  | 15  |
| Adaptabilty | 10  | 15  |

### Tank
Heavily Increased Health, Resistance and Strength, Dexterity decreased and rest of the stats aswell, looses Hunger quicker.
#### Start Stats - 44 Points
| Name        | Min | Max |
|-------------|-----|-----|
| Health      | 14  | 22  |
| Strength    | 14  | 22  |
| Resistance  | 12  | 20  |
| Dexterity   | 5   | 10  |
| Magic       | 0   | 0   |
| Ranged      | 3   | 8   |
| Hunger      | 5   | 10  |
| Adaptabilty | 15  | 20  |

### Larry
Starts with HEAVILY decreased Stats in ALL ranges. (Might still be worth it :P)
#### Start Stats - 103 Points
| Name        | Min | Max |
|-------------|-----|-----|
| Health      | 2   | 16  |
| Strength    | 2   | 16  |
| Resistance  | 2   | 16  |
| Dexterity   | 6   | 20  |
| Magic       | 0   | 10  |
| Ranged      | 2   | 12  |
| Hunger      | 2   | 15  |
| Adaptabilty | 2   | 16  |

### Joker
Randomized Stats, could start with the worst or best character

## Stats
### Health
Increases max Health, at certain level can increase Regeneration
### Strength
Increases Melee Attack damage
### Resistance
Decreases Damage from any kind of Attack
### Dexterity
Speed, Autostep, Can climb up to 2 blocks at high level.
### Magic
Can use spells based on Magic level. Also Mana Increase.
### Ranged
More damage with Ranged weapons, Bow Abilities, and Critical hit Chance which goes through Armor
### Hunger
Higher the Stat the slower Hunger Increases
### Adaptabilty
Wont be affected by different Enviourments. Heat/Cold Affects you less, could cause blindness or nausea otherwise up to DEATH.
## Special Stat: Luck (Completely Random cannot be leveled.)
Affects Loot from Chests and other things in the future

# Artifacts
Artifacts will usually Drop when fighting Bosses or in Serverwide Quests. 
They are Rare and Expensive!
## Artifact Example
The Head of King Cruel the Swine Pig.
A Head of a Piglin boss which dropped after a large fight. Only one Knight had the oppurtunity to pick it up and now has +2 stat Points in Damage.
Multiple Other Knights are trying to buy it off him with offers in the double digit Netherite Range.
